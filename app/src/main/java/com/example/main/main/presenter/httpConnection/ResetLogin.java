...
import com.example.main.main.model.apiServices.ApiResetAuthentication;
...
public class ResetLogin {
    ...
    public ResetLogin(ResetPresenter resetPresenter) {
        this.resetPresenter = resetPresenter;
    }

    public void OnLoginCompleted(AccessToken token){
        resetPresenter.OnAccessTokenReceived(token);
    }
    public void exceute() {
...
//
        ...
        ApiResetAuthentication apiResetAuthentication = retrofit.create(ApiResetAuthentication.class);


        // start asking
        Call<AccessToken> call = apiResetAuthentication.getAccesToken(user);
