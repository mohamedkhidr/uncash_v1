...
public class PermissionHandlerImpl implements PermissionHandler {
    ...
    public static void requestPermission(Model iModel , Context permissionContext, Activity permissionActivity, String accessPermission) {
        ...
        if (ActivityCompat.checkSelfPermission(context, permission) == PackageManager.PERMISSION_GRANTED) {
            isGranted = true;
            Log.v("Message : ", "permissin granted method");
            model.onPermissionRequestCompleted(true);


        } else {
            Log.v("Message : ", "not granted ");
                ...
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    if (ActivityCompat.shouldShowRequestPermissionRationale(activity, Manifest.permission.ACCESS_FINE_LOCATION)) {
                        new AlertDialog.Builder(context).
                                ...
                                setMessage("You need to grant location permission to use location" +
